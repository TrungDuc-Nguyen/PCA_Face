...
# def GetMatrixD():
path = "D:/code/python/asignments/datashet"
D = []
...
np.save("list_image", list_image)
D = np.array(D)
np.save("image_vector", D)
...
# np.save("e_vector", EV)
# X = np.load("X.npy")
# e = np.load("e_value.npy")
# EV = np.load("e_vector.npy")
# x1, x2 = X.shape
#
# print("x", X, x1, x2)
# e1 = e.shape
...
pca = np.array(pca)
pca = pca.T  # ko gian pca voi 16kx20
np.save("pca", pca)
